...
__INLINE void llm_util_aa_gen(uint8_t *acc_addr)
{
    ...
    acc_addr[0] = (LLM_AA_CT2[llm_le_env.aa.ct2_idx] << 4 ) | (llm_le_env.aa.intrand & 0x0F) | (1 << (prand & 3)); // ensures a 3rd bit set to 1
    acc_addr[1] = (LLM_AA_CT2[llm_le_env.aa.ct2_idx] << 4 ) | (llm_le_env.aa.intrand >> 4);
    acc_addr[2] = (LLM_AA_CT2[llm_le_env.aa.ct2_idx] << 4 ) | (llm_le_env.conhdl_alloc & 0x0F); // ensures at least 1 bit different from & between Advertising AAs
    acc_addr[3] = (LLM_AA_CT1[llm_le_env.aa.ct1_idx] << 4 ) | (prand >> 4) | (1 << (prand & 3)); // ensures no more than six consecutive 0s

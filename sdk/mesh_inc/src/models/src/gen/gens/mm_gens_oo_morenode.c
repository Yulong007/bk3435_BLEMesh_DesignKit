                                     ...
                                     bool publish)
{
    ...
    if ((status = mm_route_buf_alloc(&p_buf_status, data_length)) == MESH_ERR_NO_ERROR)
    {
        ...
        // Prepare environment
		static  uint32_t app_send_count = 0;
		if ((app_send_count++ % 20) == 0);
		MESH_APP_PRINT_DEBUG("%s\r\n",__func__);   ///MESH_APP_PRINT_ERR -> MESH_APP_PRINT_INFO
        if (p_route_env)
        {
    ...
    {
        MESH_APP_PRINT_ERR("%s, mm_route_buf_alloc fail, status = 0x%x\r\n", __func__, status);
    }
}
...
/**
 ****************************************************************************************
 * @brief Prepare and send a Generic OnOff set message
 *
 * @param[in] p_env_oo          Pointer to Generic OnOff Server model environment
 * @param[in] publish           True if status is a publication, false if state is a response
 ****************************************************************************************
 */
__STATIC void mm_gens_oo_send_set(mm_gens_oo_env_t *p_env_oo, mm_route_buf_env_t *p_route_env,
                                     bool publish)
{
    // Pointer to the buffer that will contain the message
    mesh_tb_buf_t *p_buf_status;
	...
	if ((app_send_count++ % 20) == 0)
       ...
       // p_buf_env->opcode = MM_MSG_GEN_OO_STATUS;
       p_buf_env->opcode = MM_MSG_GEN_OO_SET;  ///191227 18:01
...
__STATIC void mm_gens_oo_publish_set(mm_gens_oo_env_t *p_env_oo)
{
    // Check if sending of publication is enabled
    if (GETB(p_env_oo->env.info, MM_TB_STATE_MDL_INFO_PUBLI))
    {
        mm_gens_oo_send_set(p_env_oo, NULL, true);
    }
}
 ...
 ****************************************************************************************
 */
extern uint16_t genoff_rx_calc;
                                     ...
                                     mm_route_buf_env_t *p_route_env)
{
    ...
    do
    {
		...
		gpio_set(0x12, onoff);
		#else
        gpio_set(0x10, onoff);  ///add 191230
        gpio_set(0x33, onoff);
		#endif
	...
	//MESH_APP_PRINT_INFO("source addr = 0x%x,genoff_rx_calc = %d\n",p_route_env->u_addr.src ,genoff_rx_calc);
        ...
        if (p_buf->data_len == MM_GEN_OO_SET_LEN)
        {
            ...
            //delay = *(p_data + MM_GEN_OO_SET_DELAY_POS); //0228
        }
...
__STATIC void mm_gens_oo_cb_tmr_publi(void *p_env)
{
	...
	#if LED_SHOW_P1_ENABLE
	gpio_set(0x10, p_env_oo->onoff);
	gpio_set(0x11, 1); 
       gpio_set(0x12, 1);
	#else
    gpio_set(0x32, p_env_oo->onoff); ///add 200110
	#endif
    ...
    if (p_env_oo->publi_period_ms)
    {
        ...
        mm_gens_oo_publish_set(p_env_oo);

        // Restart the timer
        //if(gen_onoff_tx_nb <  (GEN_ONOFF_TX_MAX-1))
        if(gen_onoff_tx_nb < (send_nb-1) || (send_nb == 0xffff))
        {
        	mesh_tb_timer_set(&p_env_oo->tmr_publi, p_env_oo->publi_period_ms);
		gen_onoff_tx_nb++;
		MESH_APP_PRINT_INFO("T%d!",gen_onoff_tx_nb);
        }
	else
	{
	       //m_tb_store_config(20);
		//user_models_publish_clear(g_app_key_id,g_oo_mdl_lid, 0xc000);  ///add 191227 //publish set ///c000 -> gdst
		//gen_onoff_tx_nb++;
		//MESH_APP_PRINT_INFO("send END! send NB = %d\n",gen_onoff_tx_nb);
		//gen_onoff_tx_nb = 0;

		extern uint16_t gdst_temp;
		...
		m_tb_mio_get_publi_param(g_oo_mdl_lid,&gdst_temp,
								...
								&p_friend_cred_test);
	   // m_tb_store_config(20);
		//MESH_APP_PRINT_INFO("gdst_temp = 0x%4x\n",gdst_temp);
		user_models_publish_clear(g_app_key_id,g_oo_mdl_lid, gdst_temp);  ///add 191227 //publish set ///c000 -> gdst
		gen_onoff_tx_nb++;
		MESH_APP_PRINT_INFO("send END! send NB = %d\n",gen_onoff_tx_nb);
		gen_onoff_tx_nb = 0;
	}

...
{
    uint8_t sta;
    MESH_APP_PRINT_INFO("light_lightness_set, lightness %d\n", lightness);
    MESH_APP_PRINT_INFO("light is %s, prov %d\n", light_state_get() ? "ON" : "OFF", m_tb_state_get_prov_state());
#if 0
    if (lightness != 0)
    {
        if (m_tb_state_get_prov_state() == M_TB_STATE_PROV_STATE_PROV)
        {
            gpio_set(0x33, light_state_get());
            gpio_set(0x31, light_state_get());
            gpio_set(0x32, light_state_get());
        }
        MESH_APP_PRINT_INFO("light is %s, prov %d\n", light_state_get() ? "ON" : "OFF", m_tb_state_get_prov_state());
        if (light_state_get() == 1) // light is on
        {
            light_state_set(1);
...
void light_hsl_lightness_set(uint16_t lightness)
{

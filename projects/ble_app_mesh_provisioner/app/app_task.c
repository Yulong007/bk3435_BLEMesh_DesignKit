    ...
    switch (param->operation)
    {
        ...
        case (GAPM_RESET):
        {
            if (param->status == GAP_ERR_NO_ERROR)
            {
                ...
                cmd->sugg_max_tx_octets = BLE_MIN_OCTETS;//BLE_MAX_OCTETS

                cmd->sugg_max_tx_time   = BLE_MIN_TIME;//BLE_MAX_TIME
        ...
        case (GAPM_PROFILE_TASK_ADD):// 0x1B
        {
            ...
            if (!appm_add_svc())
            {
                ...
                app_mesh_reg_on_off();
                // It will used the app_mesh_enable to enable the the BLE Mesh after store the BLE Mesh info.
                app_store_mesh_info();
            }
        }
        break;
                                       ...
                                       ke_task_id_t const src_id)
{
    ...
    if (param->reason == 0x13 || param->reason == 0x08 ||param->reason == 0x28)
    {
        if (m_tb_state_get_prov_state() == M_TB_STATE_PROV_STATE_PROV)
        {
            uint16_t stu = 0;
            // This macro avoids warnings when compiling unused variables.
            UNUSED(stu);
#if (BLE_MESH_GATT_PROXY)
            stu =    m_api_proxy_ctrl(2);
            MESH_APP_PRINT_INFO("m_api_proxy_ctrl stu = 0x%x\n", stu);

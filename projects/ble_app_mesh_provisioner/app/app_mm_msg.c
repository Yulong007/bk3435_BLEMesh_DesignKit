                               ...
                               ke_task_id_t const src_id)
{
    ...
    switch (msgid)
    {
        ...
        {
            // param->ind_cod == MM_API_SRV_STATE_UPD_IND
            mm_api_srv_state_upd_ind_t *ind = (mm_api_srv_state_upd_ind_t *)param;


            if (ind->ind_code == MM_API_SRV_STATE_UPD_IND)
            {
                // MESH_APP_PRINT_INFO("MM_API_SRV_STATE_UPD_IND,state = %d,trans_time_ms = %d,state_id = %d,elmt_idx = 0x%x\n",
                //                    ind->state,ind->trans_time_ms,ind->state_id,ind->elmt_idx);
            ...
            if (ind->ind_code == MM_API_REGISTER_IND)
            {
                ...
                //MESH_APP_PRINT_INFO("model_id = 0x%x,elmt_idx = 0x%x,mdl_lid = 0x%x\n",
                //                    p_ind->model_id,p_ind->elmt_idx,p_ind->mdl_lid);
    ...
    if (trans_time_ms)
    {
        ...
        switch (state_id)
        {
            case MM_STATE_GEN_ONOFF:
            {
                ...
                MESH_APP_PRINT_INFO("state_id:MM_STATE_GEN_ONOFF\n");
            ...
            {
                uint16_t ln = (uint16_t)state;
                MESH_APP_PRINT_INFO("state_id:MM_STATE_LIGHT_CTL_LN\n");
                MESH_APP_PRINT_INFO("lightness  = %d,elmt_idx:%x\n", ln, elmt_idx);
            ...
            case MM_STATE_LIGHT_CTL_TEMP:
            {
                ...
                MESH_APP_PRINT_INFO("state_id:MM_STATE_LIGHT_CTL_TEMP\n");
            ...
            {
                uint16_t ln = (uint16_t)state;
                MESH_APP_PRINT_INFO("state_id:MM_STATE_LIGHT_HSL_LN\n");
                MESH_APP_PRINT_INFO("lightness  = %d,elmt_idx:%x\n", ln, elmt_idx);
                MESH_APP_PRINT_INFO("hsl-server--lightness = %d,Transition_Time = %d\n", ln, trans_time_ms);

                //LED_default_ctrl_time_set(trans_time_ms);
                light_hsl_lightness_set(ln);
            } break;

            case MM_STATE_LIGHT_HSL_HUE:
            {
                uint16_t hue = (uint16_t)state;
                MESH_APP_PRINT_INFO("state_id:MM_STATE_LIGHT_HSL_HUE\n");
            ...
            case MM_STATE_LIGHT_HSL_SAT:
            {
                ...
                MESH_APP_PRINT_INFO("state_id:MM_STATE_LIGHT_HSL_SAT\n");

...
static int get_input(char *inbuf, unsigned int *bp)
{
    ...
    while (cli_getchar(&c) == 1)
    {
        ...
        if (c == HCI_COMMAND ) {   /* end of input line for hci command*/ // for bk reg tools
            inbuf[*bp] = c;
            *bp = 0;
            return 1;
        }
...
static void mac_cmd(char *buf, int len, int argc, char **argv)
{
    ...
    if (argc == 1)
    {
        ...
        aos_cli_printf("ble address: %02x-%02x-%02x-%02x-%02x-%02x\r\n",
                       p_mac_l[0], p_mac_l[1], p_mac_l[2], p_mac_l[3], p_mac_u[0], p_mac_u[1]);
    }
    else if (argc == 2)
    {
        hexstr2bin(argv[1], mac.addr, 6);
...
{
    // nvds_deinit();
    aos_cli_printf("delete cmp,please reboot system!!\r\n");
}

void sys_memory_Check(void);
static void mem_check_cmd(char *buf, int len, int argc, char **argv)
{
    aos_cli_printf("mem_check\r\n");
    // sys_memory_Check();

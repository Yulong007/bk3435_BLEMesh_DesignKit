...
void bdaddr_env_init(void)
{
    ...
    co_default_bdaddr.addr[0] = 0xf8;//f8a76324a49e;
    co_default_bdaddr.addr[1] = 0xa7;
    co_default_bdaddr.addr[2] = 0x63;
    co_default_bdaddr.addr[3] = 0x24;
    co_default_bdaddr.addr[4] = 0xa4;
    co_default_bdaddr.addr[5] = 0x9e;
...
static void uart_rx_handler(uint8_t *buf, uint8_t len)
{
    if (buf[0] == 0x01)
    {
        ...
        ali_light_status_report(0x0100, 1, 1);
    }
    else if (buf[0] == 0x02)
    {
        MESH_APP_PRINT_INFO("report off\r\n");
        ali_light_status_report(0x0100, 1, 0);
    }
}
#endif

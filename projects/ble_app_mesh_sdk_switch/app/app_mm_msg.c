...
void app_socket_param_set_handle(uint32_t state, uint32_t trans_time_ms, uint16_t state_id, uint8_t elmt_idx);
    ...
    switch (msgid)
    {
        case MESH_MDL_API_IND:
        {
            ...
            if (ind->ind_code == MM_API_SRV_STATE_UPD_IND)
            {
                ...
                app_socket_param_set_handle(ind->state, ind->trans_time_ms, ind->state_id, ind->elmt_idx);
            }

...
uint8_t aes_cbc_encrypt_pkcs7(AES_CTX *c, const uint8_t key[16], const uint8_t *in, uint8_t in_len, uint8_t *out, int *out_len)
{
	...
	if (c == NULL || key == NULL || in == NULL || in_len <= 0|| out == NULL || out_len == NULL)
		return AES_INPUT_DATA_INCORRECT;
	if(in == out)
		return AES_INPUT_OUTPUT_USE_SAME_BUFFER;
	...
	while(in_len>=16)
	{
		AES_cbc_encrypt(in+offset, out+offset, 16, &c->key, c->iv);
		offset += 16;
		in_len -= 16;
		aes_counter++;
	}	
	#if 0
	if(in_len == 0)	//AES complete
	{
		*out_len = aes_counter*16;
		return AES_COMPLETE;
	}
	#endif	
	//pkcs7 padding
	...
	*out_len = (aes_counter+1)*16;
	...
	AES_cbc_encrypt(out+offset, out+offset, 16, &c->key, c->iv);
	return AES_COMPLETE;
}
...
uint8_t aes_cbc_decrypt_pkcs7(AES_CTX *c, const uint8_t key[16],const uint8_t *in, uint8_t in_len, uint8_t *out, int *out_len)
{
	...
	if(ret != 0) return AES_SET_DECRYPT_KEY_FAIL;
	
	while(in_len > 16) {
		AES_cbc_decrypt(in+offset, out+offset, 16, &c->key, c->iv);
		offset += 16;
		in_len -= 16;
		aes_counter++;
	}

...
#if (POWER_ONOFF_ONLY)
             MESH_APP_PRINT_INFO("##############light_lightness_set222222222222\n");
            gpio_set(0x10, 0);
#endif
        }
...
#if (POWER_ONOFF_ONLY)
        MESH_APP_PRINT_INFO("##############light_lightness_set3333333333333\n");
        gpio_set(0x10, 0);
#endif
    }
...
uint32_t light_state_nv_store(flash_light_param_type_t type)
{
    ...
    switch (type)
    {
        ...
        case FLASH_LIGHT_PARAM_TYPE_LIGHT_STATE:
        {
            ...
            //uart_printf("---3-light_lightness= %d,light_mode= %d,ret:%d\r\n",light_lightness,light_mode_get(),ret);
        }
        break;
